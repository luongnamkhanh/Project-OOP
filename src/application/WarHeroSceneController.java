...
import historicalFigure.WarHeroQuery;
...
public class WarHeroSceneController extends SwitchSceneController{
	@FXML
	...
	public void pressContinueButton(ActionEvent e) throws IOException {
		...
		if(battleCheckBox.isSelected()) 
			userChoice.add(true);
		else
			userChoice.add(false);

		HelloRDFWorld a = new HelloRDFWorld();
		WarHeroQuery g = new WarHeroQuery();
		String warHeroFilename = "WarHero.ttl";

		a.importData(g.constructQuery(userChoice), warHeroFilename);

		String result = "";
		File file = new File(warHeroFilename);
