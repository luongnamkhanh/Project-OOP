...
public class PoliticianSceneController extends SwitchSceneController{
	@FXML
	...
	@FXML
	private CheckBox awardCheckBox;
	...
	public void pressContinueButton(ActionEvent e) throws IOException {
		...
		if(awardCheckBox.isSelected()) 
			userChoice.add(true);
		else
			userChoice.add(false);

		HelloRDFWorld a = new HelloRDFWorld();
		PoliticianQuery g = new PoliticianQuery();
		String politicianFilename = "Politician.ttl";

		a.importData(g.constructQuery(userChoice), politicianFilename);

		String result = "";
		File file = new File(politicianFilename);
