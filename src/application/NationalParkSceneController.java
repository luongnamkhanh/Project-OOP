...
import attraction.NationalParkQuery;
...
public class NationalParkSceneController extends SwitchSceneController{
	@FXML
	...
	@FXML
	private CheckBox nearestCityCheckBox;
	@FXML
	private CheckBox areaCheckBox;
	@FXML
	private CheckBox establishedCheckBox;
	...
	public void pressContinueButton(ActionEvent e) throws IOException {
		...
		if(nearestCityCheckBox.isSelected()) 
			userChoice.add(true);
		else
			userChoice.add(false);
		...
		if(establishedCheckBox.isSelected()) 
			userChoice.add(true);
		else
			userChoice.add(false);

		HelloRDFWorld a = new HelloRDFWorld();
		NationalParkQuery g = new NationalParkQuery();
		String nationalParkFilename = "NationalPark.ttl";

		a.importData(g.constructQuery(userChoice), nationalParkFilename);

		String result = "";
		File file = new File(nationalParkFilename);
