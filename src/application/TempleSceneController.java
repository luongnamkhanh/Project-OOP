...
public class TempleSceneController extends SwitchSceneController{
	@FXML
	...
	@FXML
	private CheckBox religiousAffiliationCheckBox;
	@FXML
	private CheckBox yearCompletedCheckBox;
	...
	public void pressContinueButton(ActionEvent e) throws IOException {
		...
		if(religiousAffiliationCheckBox.isSelected()) 
			userChoice.add(true);
		else
			userChoice.add(false);

		if(yearCompletedCheckBox.isSelected()) 
			userChoice.add(true);
		else
			userChoice.add(false);

		HelloRDFWorld a = new HelloRDFWorld();
		TempleQuery g = new TempleQuery();
		String templeFilename = "Temple.ttl";

		a.importData(g.constructQuery(userChoice), templeFilename);
